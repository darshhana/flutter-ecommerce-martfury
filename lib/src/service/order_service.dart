...
class OrderService extends BaseService {
  Future<List<Order>> getOrders() async {
    try {
      final response = await get('/ecommerce/orders');
  ...
  Future<Map<String, dynamic>> trackOrder({
    ...
    try {
      ...
      final response = await post('/ecommerce/orders/tracking', {
        'code': cleanCode,
        'email': email,
      });
      ...
      try {
        final response = await get('/orders/tracking?code=${Uri.encodeComponent(code)}&email=${Uri.encodeComponent(email)}');
